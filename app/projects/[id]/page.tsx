...
export default function ProjectDetail({ params }: Props) {
  ...
  // 🔧 Route param-аас project-оо олох (id эсвэл title-slug-аар тааруулна)
  const slugify = (s: string) => s.toLowerCase().replace(/\s+/g, "-");
  const index = projects.findIndex(
    (p) => String(p.id) === id || slugify(p.title) === id,
  );
  ...
  const goNext = useCallback(() => {
    ...
    setActiveImage(0);
    // 🔧 Route path-аа энд тааруул (одоо /projects/[id] гэж үзсэн)
    ...
    return (
      <div className="fixed inset-0 z-[100] flex items-center justify-center bg-white">
        <div className="text-center">
          ...
          <button
            onClick={() => router.push("/")}
            className="text-sm border-b border-black pb-1 hover:opacity-50 transition"
          >
            ← Back
          </button>
        </div>
        ...
        <div className="flex flex-col md:flex-row">
          ...
          <div className="relative z-0 w-full md:w-[36%] px-8 md:px-10 py-10 md:py-14 bg-white">
            ...
            >
              {[
                {
                  label: t.projects.modal.client,
                  value: project.detail.client,
                },
