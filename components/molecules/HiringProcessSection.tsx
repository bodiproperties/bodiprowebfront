...
export function HiringProcessSection({ lang }: HiringProcessSectionProps) {
  ...
  return (
    <section id="process" className="bg-neutral-900 text-white py-24 sm:py-32 px-6 sm:px-8 relative overflow-hidden">
      ...
      <div className="max-w-6xl mx-auto relative z-10">
        <Reveal direction="left">
          ...
          <h2 className="mt-4 text-3xl sm:text-4xl md:text-5xl font-extralight mb-6">
            {lang === 'mn' ? 'Сонгон Шалгаруулалтын Үе Шатууд' : 'Hiring Process'}
          </h2>
          <p className="text-neutral-400 font-light max-w-2xl text-base sm:text-lg mb-16">
            {lang === 'mn'
                ...
                >
                  <div>
                    <div className="flex justify-between items-center mb-6">
                      <span className={`text-5xl font-extralight transition-colors ${isSelected ? 'text-[#F58220]' : 'text-white/20'}`}>
                        {step.number}
                      </span>
                      {isSelected && <CheckCircle2 className="w-5 h-5 text-[#F58220]" />}
                    </div>

                    <h3 className="text-xl font-light text-white mb-2">{step.title[lang]}</h3>
                    <p className="text-xs uppercase tracking-wider text-[#F58220] font-medium mb-4">
                      {step.subtitle[lang]}
                    </p>
                  ...
                  <div className="pt-4 border-t border-neutral-800 flex items-center justify-between text-xs text-neutral-500 font-mono">
        ...
        <div className="mt-12 bg-neutral-800/80 p-6 sm:p-8 border border-neutral-700/60 flex flex-col sm:flex-row items-start sm:items-center gap-6">
          ...
          <div className="flex-1">
            <h4 className="text-sm font-semibold uppercase tracking-wider text-white">
              {lang === 'mn' ? `Шаталбар ${HIRING_STEPS[activeStep].number} Зөвлөгөө:` : `Step ${HIRING_STEPS[activeStep].number} Candidate Tip:`}
            </h4>
